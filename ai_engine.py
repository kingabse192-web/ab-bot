...
class AIEngine:
    ...
    def respond(self, uid, msg, bot=None, chat_id=None):
        ...
        # ── Dual-stage AI: search → synthesize ──
    ...
    def _cmd_help(self, uid):
        return (
            "*ab — Your AI*\n\n"
            "*Chat normally:*\n"
            "Ask anything — I search Wikipedia, DuckDuckGo, and the web!\n\n"
            "*Generate:*\n"
            "`imagine [description]` — generate an AI image\n\n"
            "*Media:*\n"
            "`movie [name]` — find movie info + voice\n"
            "`song [name]` — find song info + voice\n"
...
"""
    ...
    def _query_free_ai(self, uid, msg, search_context=""):
        try:
            ...
            if search_context:
                prompt = f"You are ab, a helpful AI assistant. The user's name is {name}. Their mood seems {mood}. Rules: {'; '.join(rules)}.\n\nHere is information from a web search:\n{search_context[:1500]}\n\nUse this information to answer the user's question naturally and concisely.\n\nUser: {msg}\nYou:"
            else:
                prompt = f"You are ab, a helpful AI assistant. The user's name is {name}. Their mood seems {mood}. Rules: {'; '.join(rules)}. Recent context: {ctx[:300]}\n\nUser: {msg}\nYou (answer concisely and helpfully):"
            data = json.dumps({"inputs": prompt, "parameters": {"max_new_tokens": 350, "temperature": 0.7}}).encode()
            req = Request("https://api-inference.huggingface.co/models/microsoft/Phi-3-mini-4k-instruct",
                          data=data, headers={"Content-Type": "application/json"})
            ...
            if isinstance(result, list) and result:
                ...
                if text:
                    markers = ["You (answer concisely and helpfully):", "You:"]
                    for m in markers:
                        idx = text.find(m)
                        if idx >= 0:
    ...
    def _multi_source_answer(self, uid, msg):
        ...
        if sources:
            ...
            self.check_ollama()
            if self.ollama_ready:
                ai_reply = self._query_ollama(uid, msg, f"Here are results from multiple sources (Wikipedia, DuckDuckGo, Google, Web, AI, Owner's Memory):\n\n{all_text[:2000]}\n\nSynthesize these into one clear answer for the user.")
                if ai_reply:
                    return ai_reply
            ai_reply = self._query_free_ai(uid, msg, f"Here are results from multiple sources:\n\n{all_text[:2000]}\n\nCombine these into one clear concluded answer.")
            if ai_reply:
                return ai_reply
